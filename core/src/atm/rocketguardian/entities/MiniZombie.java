...
public class MiniZombie extends Actor implements Zombie {
	...
	public MiniZombie(World world, Vector2 position) {
		// Assign world and other properties
		this.world = world;
		this.sprite = new Sprite(AssetLoader.miniZombieAnimation.getKeyFrame(0));
		this.stateTime = 0;
		this.isAlive = true;
		...
		// box, but in pixels.
		setSize(0.86f * Constants.PPM, Constants.PPM);
	...
	public void act(float delta) {
		...
		setPosition((body.getPosition().x - 0.43f) * Constants.PPM, (body.getPosition().y - 0.5f) * Constants.PPM);

		// Set the sprite coordinates and dimensions to match its Box2D body.
		sprite.setPosition(getX(), getY());
		sprite.setSize(getWidth(), getHeight());
		sprite.setRegion(AssetLoader.miniZombieAnimation.getKeyFrame(stateTime));
