...
public class Baby extends Actor implements Human {
	private World world;
	private Body body;
	...
	public Baby(World world, Vector2 position) {
		...
		this.world = world;
		// At first, the Baby is created with a random animation. It will be
		...
		// Baby sprite dimensions are 1.15x0.86 meters. Box2D requires
		// half-width
		// and half-height to set box dimensions, so we divide it by 2.
		PolygonShape shape = new PolygonShape();
		shape.setAsBox(0.52f, 0.38f);
		...
		// box, but in pixels.
		setSize(0.52f * 2 * Constants.PPM, 0.38f * 2 * Constants.PPM);
	...
	public void act(float delta) {
		...
		setPosition((body.getPosition().x - 0.52f) * Constants.PPM, (body.getPosition().y - 0.38f) * Constants.PPM);
	...
	private List<Animation<TextureAtlas.AtlasRegion>> createAnimationsList() {
		...
		res.add(AssetLoader.blackBabyAnimation);
		res.add(AssetLoader.whiteBabyAnimation);
		return res;
	}
